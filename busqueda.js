const listarCategorias = async()=>{
    ...
    tituloCat.classList.add("text-center");
      let query = params.q.replace('%20', ' ');

    const data = new FormData();
    data.append('query',query );
  
  
      const peticion = await fetch('class/listarResultadosBusqueda.php',{
      method: 'POST',
      body: data
  })
     const resp = await peticion.text();
     console.log(resp);
    ...
    //  mostrarCategoriasAside();
        ...
        if(cat.categoria == undefined) {
          ...
          if(resp ==='vacios'){
            textoMensaje.innerHTML = `No hay resultados para tus busqueda `;
